...
export default class ErrorHandler {
    ...
    static handleGatewayError = (error: any): GatewayResponse => {
        ...
        const eFromCode = error instanceof ErrorWithCode ? error : errorFromCode(errorCode ?? error);
        return {
            code: eFromCode?.code ?? errorCode ?? -1,
            description: eFromCode?.message ?? eFromCode?.name ?? error?.message ?? 'UNKNOWN_ERROR',
            language: 'EN',
        };
    }
